...
커밋 푸시는 CI만 실행하며 npm 게시는 수행하지 않습니다. 관리자가 게시할 커밋을 선택해 [Publish 워크플로](./.github/workflows/publish.yml)를 수동 실행합니다. 모든 공개 워크스페이스 패키지를 확인하고, npm에 이미 있는 버전은 건너뛰며, 없는 버전을 의존성 순서로 게시한 뒤 해당 커밋에 패키지별 버전 태그와 GitHub 릴리스를 생성합니다. 일부 게시 후 실패해도 게시한 버전을 되돌리지 않으며, 재실행하면 성공한 npm 게시를 건너뜁니다. 패키지별 npm 게시 권한이 필요하고 워크플로는 Trusted Publishing을 사용합니다. 신규 패키지나 신뢰 설정이 없는 패키지는 무인 배포 전에 관리자의 초기 설정이 필요합니다.

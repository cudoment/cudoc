...
두어 `body`와 `header`의 오른쪽에 패널 폭(22rem)만큼 안쪽 여백을 주므로 사이드바와
목차가 가려지지 않고, *본문 위에 겹침*은 패널을 그 위에 덮습니다. 768px 미만에서는

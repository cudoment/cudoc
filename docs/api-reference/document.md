...
| `tableColumnWidths`        | `[]`                           | Minimum column widths by header text, applied before layout   |
| `ignoreDiagnostics`        | `[]`                           | Diagnostic codes not reported; the nodes are handled the same |
...
`DocumentDiagnostic` has `code`, `message`, optional unist `position`. Current nonfatal codes are `UNKNOWN_CALLOUT_TYPE` and `DYNAMIC_COMPONENT`. Callout type registration requires names matching `/^[a-z][\w-]*$/i`. Unknown cudoc callout markers remain intact; dynamic mapped/native JSX is retained instead of evaluated. `DYNAMIC_COMPONENT` fires when a mapped component or a lowered native tag carries an attribute whose value is an expression naming something (`rows={count}`); a literal in braces (`rows={2}`) is static. `ignoreDiagnostics: ["DYNAMIC_COMPONENT"]` drops such a code from the result in a project whose components take expressions by design — the tree is identical with or without the option, only the message is gone — and a non-string entry throws. Adapters accept the same key.
...
Other semantics include `heading` (`explicitId`, optional `badge`), `badge` (rendered `span`), `permalink` and `pageBreak`: a ` ```cudoc-pagebreak ` fence becomes a `thematicBreak` carrying `hName: "div"`, `className: ["cudoc-page-break"]`, `hidden: true` and `cudoc.kind: "pageBreak"`, so it renders nothing on a site and breaks the page in a paginated export ([shared constants](./adapters.md#paginated-output)). Heading IDs live in `data.hProperties.id`. `DocumentNode` is an extensible structural type, not a guarantee that every custom node is renderable. `DocumentData` also allows other host metadata.
...
A consumer that reads stored trees — a search indexer, a dataset generator, a renderer of its own — can rely on the following under version 1, and the version is raised when any of it changes rather than when a node kind is added. A `<br>` or `<br />` in prose or in a table cell that is not a cell list is the mdast `break` node, never a JSX element. Heading IDs are in `data.hProperties.id`; the `Anchor` element with a `headerLevel` attribute exists only in the low-level component transforms and never in a stored tree, because normalization lowers it. Document IDs are library paths without the extension, so with `roots` they start with the root's base (`docs/guide/setup`), and every link, embed source and dataset scope uses that form. `mdxjsEsm`, `position` and `estree` are absent; expression nodes keep their `value` text. Callouts, badges, permalinks and page breaks are the `data.cudoc` kinds above. `private` and `imports` live on the manifest entry, not in the tree.
...
Source: [styles.css](../../packages/cudoc/styles.css), imported as `@cudoment/cudoc/styles.css`. It styles the `cudoc-callout`, `cudoc-callout-title` and `cudoc-badge` class names the renderer emits, and nothing else.

Because it loads inside a host's own page it sets no text colour: body text inherits the host's theme, and only surfaces and accents flip. Its properties are namespaced `--cudoc-wash`, `--cudoc-line`, `--cudoc-accent`, `--cudoc-accent-soft`, `--cudoc-warn`, `--cudoc-warn-wash`, `--cudoc-danger` and `--cudoc-danger-wash`, so a site retheme the components by redefining those without touching host variables. Three theme signals are honoured, in this order of specificity: `prefers-color-scheme: dark`, a `dark` class on the root element (VitePress, Nextra), and `data-theme="dark"` (Docusaurus). A `light` class or `data-theme="light"` restores the light palette even when the system prefers dark, so a host's own toggle wins in both directions. The palette matches `siteStyles` in [the export adapter](./adapters.md#export), and every surface clears WCAG AA against both a light and a dark inherited text colour.
...
| [/mdx](../../packages/cudoc/src/internal/core/mdx/index.ts)                                                        | Attribute/flow/text element creation, attribute reading/writing, line-break and child conversion helpers; `importedNames(tree)` and `importedNamesFromSource(source)` for the local names a document's `import` statements bind |
...
`TableColumnWidthOptions` (`tableColumnWidths`) is `{ section?, widths, metadataDelimiters?, ignoreElements? }`: `widths` maps header text — compared after the heading metadata and ignored elements such as a badge are removed and spaces collapsed — to a CSS length (`120px`, `6ch`, `20rem`, `25%`; anything else, an empty map or a non-object rule throws naming the option path). A rule without `section` reaches every table. Each matching header cell gets `data.hProperties.style: "min-width: <length>"`, exactly what an embed table column's `minWidth` writes, so the HTML site, the paginated formats and the Word writer treat both the same way ([Word column widths](./adapters.md#word-template)). Width rules run before layout rules: a layout rule rebuilds the header cells it matches as elements and carries the cell's style into the element's `style` prop merged with the column alignment (`{ textAlign, minWidth }`), and normalization lowers that back to the flattened `text-align:center;min-width:18rem` string. Header cells the rule does not name keep whatever style they had.

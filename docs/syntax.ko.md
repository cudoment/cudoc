...
일정 폭보다 좁아지면 안 되는 열에는 셀마다 `<div style>`을 쓰는 대신 폭 규칙을 지정합니다.
...
헤더 글자가 열을 고르고 CSS 길이가 그 헤더 셀의 `min-width`가 되며, `section`이 없는 규칙은 모든 표에 적용됩니다. 폭 규칙과 배치 규칙은 같은 열에 함께 쓸 수 있습니다. 참고: [문서 옵션](./api-reference/document.ko.md#문서-옵션).

## 페이지 나누기

정보 문자열이 `cudoc-pagebreak`인 코드 펜스는 페이지를 나누는 출력에서 쪽을
넘깁니다. 문서 사이트에서는 아무것도 보이지 않습니다.
...
펜스는 파서를 확장하지 않고도 여섯 호스트의 컴파일러를 모두 그대로 통과하는
유일한 블록 형태입니다. HTML 주석은 MDX에서 컴파일되지 않고, 링크 정의를
이용하는 방법은 markdown-it이 토큰을 조용히 버리기 때문에 저자가 지정한 나누기가
절반의 호스트에서 아무 진단 없이 사라집니다.

**이 문법은 `host | cudoc | both`를 고르는 여섯 번째 기능이 아닙니다.** 그 축은
같은 의미를 cudoc 철자와 호스트 고유 철자 중 무엇으로 쓸지 고르는 것인데, 여섯
호스트 가운데 페이지 나누기 철자를 가진 곳이 없습니다. 그래서 `pageBreak: "host"`는
"아무것도 인식하지 않음"을 뜻하게 되고 본문에 빈 코드 블록이 그대로 보이게 됩니다.
이 표시를 존중할지는 문법이 아니라 출력이 정합니다. 쪽을 나누는 내보내기는
`page.authoredBreaks`를 `false`로 두지 않는 한 존중하고, 사이트는 아무것도 보여
주지 않습니다. 코드 펜스로 쓰는 임베드도 같은
이유로 `SyntaxOptions` 밖에 있습니다.

페이지를 나누는 출력은 스스로도 쪽을 넘깁니다. 한 권으로 묶은 파일에서 문서가
시작할 때, 제목이 쪽 끝에 혼자 남지 않도록 할 때, 그리고 표의 행 중간에서는
절대 넘기지 않습니다. 저자가 직접 지정하는 나누기는 이런 규칙이 알 수 없는
자리를 위한 것입니다.

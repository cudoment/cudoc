...
제한 섹션의 제목, 본문, 하위 섹션을 포함합니다. 같은 단계 또는 상위 단계의 다음 제목 직전까지가 섹션입니다. `#limits`를 생략하면 문서 전체를 가져오며 확장자도 생략할 수 있습니다. 경로는 임베드를 작성한 문서 기준입니다. `/`로 시작하면 운영체제 루트가 아닌 `sourceRoot` 기준입니다. 수집된 로컬 문서만 대상으로 지정할 수 있습니다.
...
선택한 **원본 Markdown 소스**에 규칙을 순서대로 적용합니다. 기본적으로 하위 섹션도 포함합니다. 일반 문자열 규칙은 일치하는 모든 부분을 바꾸고, 정규식은 `regex: true`로 활성화하며 기본 플래그는 `g`입니다. Markdown 표기를 유지한 채 원본 문서의 컴파일러 설정으로 다시 컴파일합니다. 원본 파일과 수집된 AST는 수정하지 않습니다.

선택한 콘텐츠가 참조하는 외부 참조 정의도 사용할 수 있습니다. 치환 대상은 선택한 원문 범위이며, 그 범위 밖에서 추가한 정의는 치환하지 않습니다.
